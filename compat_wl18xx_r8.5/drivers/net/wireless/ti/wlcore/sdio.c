					      ...
					      void *buf, size_t len, bool fixed)
{
	...
	sdio_claim_host(func);
	//printk(KERN_WARNING "wl12xx_sdio_raw_write @1,fixed=%d\n",fixed);
	if (unlikely(dump)) {
	...
	if (unlikely(addr == HW_ACCESS_ELP_CTRL_REG)) {
		printk(KERN_WARNING "wl12xx_sdio_raw_write @2\n");
		...
		dev_dbg(child->parent, "sdio write 53 addr 0x%x, %zu bytes\n",
			addr, len);
		printk(KERN_WARNING "wl12xx_sdio_raw_write @3 sdio write 53 addr 0x%x, %zu bytes\n",addr,len);
		if (fixed){
			printk(KERN_WARNING "@ fixed");			
			ret = sdio_writesb(func, addr, buf, len);
		}else{
			//printk(KERN_WARNING "@ non-fixed"); 
...
static struct wl12xx_platform_data *get_platform_data(struct device *dev)
{
	...
	if (pdata){
		printk(KERN_WARNING "got platform data from DT, pdata->board_ref_clock=%d,pdata->board_tcxo_clock=%d,pdata->platform_quirks=%d\n",pdata->board_ref_clock,pdata->board_tcxo_clock,pdata->platform_quirks);
		return pdata;
	}
...
{
	printk(KERN_ERR "wl1271_init call\n");
